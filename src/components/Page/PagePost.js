...
const PagePostEditor = Loadable({
  loader: () => import(/* webpackChunkName: "PagePostEditor" */'./PagePostEditor'),
  loading: () => <LDCircular />
});
...
export default function PagePost(props){
  ...
  const pageid = ( split[splitLength - 2] === 'post' && split[splitLength - 3] === 'user' ) ? parseInt(split[splitLength - 1]) : null
          ...
          API.handleSortArrayByDate(data, 'createdate', 'title').map( d =>{
            ...
            <React.Fragment key={d.postid}>
              <ListItem button>
                <ListItemAvatar style={{ marginRight: 16 }}>
                  {
                    d.photopath?
                      <Avatar
                        ...
                        src={
                          isSupportWebp?
                          currentWebURL + d.photopath + '.webp#' + new Date().toISOString()
                          :
                          currentWebURL + d.photopath + '.jpg#' + new Date().toISOString()
                        }
                ...
                <ListItemText className={classes.listDetail} primary={d.message} secondary={d.submessage}/>
                <ListItemIcon>
                  { editting?
